...
prompt_message = [
    ...
    {"role": "system", "content": "You will generate paraphrases of the sentence and its translation in Finnish language."},
    {"role": "system", "content": "You will generate a translation of input sentence in Finnish, and also generate 4 paraphrases and its translaton in Finnish."},
    ...
    {"role": "system", "content": "Translation: Mailassa olevat miehet ovat valmiita heilumaan kentällä, kun tuomari katsoo.\n\
Paraphrase 1: The male players at the bat ready to hit the ball as the umpire watches attentively. / Miespelaajat mailalla valmiina lyömään palloa, kun erotuomari katselee tarkkaavaisesti.\n\
Paraphrase 2: The male batters at the bat prepare to hit the pitch as the umpire stands watch. / Mailassa olevat uroslyönnit valmistautuvat iskemään kentälle, kun tuomari seisoo katsomassa.\n\
Paraphrase 3: The batters at the plate are poised to swing as the umpire keeps an eye on them. / Lautanen vatkat ovat valmiita heilumaan, kun tuomari pitää heitä silmällä.\n\
Paraphrase 4: The hitters at the plate wait for themselves to take their swings at the ball while the umpire looks on. / Lautasen lyöjät odottavat itseään, että he heilaavat palloa, kun erotuomari katsoo.\n"},
...
def gpt_annotating_multiprocess_fi(args: argparse.Namespace) -> None:
    # Define data_dict
    ...
    for result in results:
        ...
        data_dict_fi['image_names'] += result[1]['image_names'] # result[1] is data_dict_fi
        data_dict_fi['captions'] += result[1]['captions']
        data_dict_fi['all_captions'] += result[1]['all_captions']
        data_dict_fi['caption_numbers'] += result[1]['caption_numbers']
    ...
    for idx in tqdm(range(len(data_dict_fi['captions'])), desc='Tokenizing Finnish captions'):
        cap = data_dict_fi['captions'][idx]
        tokenized = fi_tokenizer(cap, padding='max_length', truncation=True,
                                  max_length=args.max_seq_len, return_tensors='pt')
        data_dict_fi['input_ids'].append(tokenized['input_ids'].squeeze())
    ...
    assert len(data_dict_en['image_names']) == len(data_dict_fi['image_names']), f"len(data_dict_en['image_names']):{len(data_dict_en['image_names'])} != len(data_dict_fi['image_names']):{len(data_dict_fi['image_names'])}"
    assert len(data_dict_fi['image_names']) == len(data_dict_fi['captions']) == len(data_dict_fi['all_captions']) == len(data_dict_fi['caption_numbers']) == len(data_dict_fi['input_ids']), f"data_dict_fi lengths are not equal"

    # Save data_dict_en & data_dict_fi as pickle file
    if args.gpt_model_version == 'gpt-3.5-turbo':
        save_name_en = 'train_GPT35_EN.pkl'
        save_name_fi = 'train_GPT35_FI.pkl'
    elif 'gpt-4' in args.gpt_model_version:
        save_name_en = 'train_GPT4_EN.pkl'
        save_name_fi = 'train_GPT4_FI.pkl'
    ...
    with open(os.path.join(preprocessed_path, save_name_fi), 'wb') as f:
        pickle.dump(data_dict_fi, f)
        print(f"Saved {save_name_fi} in {preprocessed_path}")
    ...
    for idx in range(len(image_names_sublist)):
        ...
        while True:
            try:
                ...
                result_sentences = []
                result_sentences.append({"en": gold_caption, "fi": gpt_sentences[0]})
                for i in range(1, len(gpt_sentences)):
                    result_sentences.append({"en": gpt_sentences[i].split(" / ")[0], "fi": gpt_sentences[i].split(" / ")[1]})
            except KeyboardInterrupt as k:
        ...
        # Append to data_dict_en & data_dict_fi
        for i in range(len(result_sentences)):
            ...
            # Append to data_dict_fi
            subset_dict_fi['image_names'].append(image_name)
            subset_dict_fi['captions'].append(result_sentences[i]['fi'])
            subset_dict_fi['caption_numbers'].append(i+1)
            subset_dict_fi['all_captions'].append([result_sentences[i]['fi'] for i in range(len(result_sentences))])

        if tqdm_bar.n + NUM_PROCESS <= tqdm_bar.total:
            tqdm_bar.update(NUM_PROCESS)

    return subset_dict_en, subset_dict_fi # return data_dict_en & data_dict_fi
