...
prompt_message = [
    ...
    {"role": "system", "content": "You will generate paraphrases of the sentence and its translation in Polish language."},
    {"role": "system", "content": "You will generate a translation of input sentence in Polish, and also generate one paraphrase and its translaton in Polish."},
    ...
    {"role": "system", "content": "Translation: Mężczyźni odbijający są gotowi do huśtania się na boisku, podczas gdy sędzia patrzy.\n\
Paraphrase: The male players at the bat ready to hit the ball as the umpire watches attentively. / Gracze płci męskiej przy pałce są gotowi do uderzenia piłki, a sędzia uważnie się temu przygląda.\n"},
...
def gpt_annotating_multiprocess_pl(args: argparse.Namespace) -> None:
    # Define data_dict
    ...
    #assert len(train_data_dict['image_names']) == total_length // 2, f"len(train_data_dict['image_names']):{len(train_data_dict['image_names'])} != total_length // 2:{total_length // 2}"
    if len(train_data_dict['image_names']) != total_length // 2:
        #raise UserWarning(f"len(train_data_dict['image_names']):{len(train_data_dict['image_names'])} != total_length // 2:{total_length // 2}")
        print(f"len(train_data_dict['image_names']):{len(train_data_dict['image_names'])} != total_length // 2:{total_length // 2}")
    ...
    for result in results:
        ...
        data_dict_pl['image_names'] += result[1]['image_names'] # result[1] is data_dict_pl
        data_dict_pl['captions'] += result[1]['captions']
        data_dict_pl['all_captions'] += result[1]['all_captions']
        data_dict_pl['caption_numbers'] += result[1]['caption_numbers']
    ...
    for idx in tqdm(range(len(data_dict_pl['captions'])), desc='Tokenizing Polish captions'):
        cap = data_dict_pl['captions'][idx]
        tokenized = pl_tokenizer(cap, padding='max_length', truncation=True,
                                  max_length=args.max_seq_len, return_tensors='pt')
        data_dict_pl['input_ids'].append(tokenized['input_ids'].squeeze())
    ...
    assert len(data_dict_en['image_names']) == len(data_dict_pl['image_names']), f"len(data_dict_en['image_names']):{len(data_dict_en['image_names'])} != len(data_dict_pl['image_names']):{len(data_dict_pl['image_names'])}"
    assert len(data_dict_pl['image_names']) == len(data_dict_pl['captions']) == len(data_dict_pl['all_captions']) == len(data_dict_pl['caption_numbers']) == len(data_dict_pl['input_ids']), f"data_dict_pl lengths are not equal"

    # Save data_dict_en & data_dict_pl as pickle file
    if args.gpt_model_version == 'gpt-3.5-turbo':
        save_name_en = 'train_GPT35_EN.pkl'
        save_name_pl = 'train_GPT35_PL.pkl'
    elif args.gpt_model_version == 'gpt-4':
        save_name_en = 'train_GPT4_EN.pkl'
        save_name_pl = 'train_GPT4_PL.pkl'
    ...
    with open(os.path.join(preprocessed_path, save_name_pl), 'wb') as f:
        pickle.dump(data_dict_pl, f)
        print(f"Saved {save_name_pl} in {preprocessed_path}")
    ...
    for idx in range(len(image_names_sublist)):
        ...
        while True:
            try:
                ...
                result_sentences = []
                result_sentences.append({"en": gold_caption, "pl": gpt_sentences[0]})
                for i in range(1, len(gpt_sentences)):
                    result_sentences.append({"en": gpt_sentences[i].split(" / ")[0], "pl": gpt_sentences[i].split(" / ")[1]})
            except KeyboardInterrupt as k:
            ...
            if len(result_sentences) == 2: # if gpt_response is correctly generated and has 2 sentences
        ...
        # Append to data_dict_en & data_dict_pl
        for i in range(len(result_sentences)):
            ...
            # Append to data_dict_pl
            subset_dict_pl['image_names'].append(image_name)
            subset_dict_pl['captions'].append(result_sentences[i]['pl'])
            subset_dict_pl['caption_numbers'].append(i+1)
            subset_dict_pl['all_captions'].append([result_sentences[i]['pl'] for i in range(len(result_sentences))])

        if tqdm_bar.n + NUM_PROCESS <= tqdm_bar.total:
            tqdm_bar.update(NUM_PROCESS)

    return subset_dict_en, subset_dict_pl # return data_dict_en & data_dict_pl
