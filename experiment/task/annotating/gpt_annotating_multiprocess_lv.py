...
prompt_message = [
    ...
    {"role": "system", "content": "You will generate paraphrases of the sentence and its translation in Latvian language."},
    {"role": "system", "content": "You will generate a translation of input sentence in Latvian, and also generate 4 paraphrases and its translaton in Latvian."},
    ...
    {"role": "system", "content": "Translation: Vīrieši pie nūjas ir gatavi šūpoties laukumā, kamēr tiesnesis skatās.\n\
Paraphrase 1: The male players at the bat ready to hit the ball as the umpire watches attentively. / Vīrieši pie nūjas ir gatavi sist bumbiņu, kamēr tiesnesis uzmanīgi vēro.\n\
Paraphrase 2: The male batters at the bat prepare to hit the pitch as the umpire stands watch. / Vīrieši pie nūjas gatavojas sist laukumā, kamēr tiesnesis stāv, skatās.\n\
Paraphrase 3: The batters at the plate are poised to swing as the umpire keeps an eye on them. / Mīklas pie šķīvja ir gatavi šūpoties, kamēr tiesnesis viņus vēro.\n\
Paraphrase 4: The hitters at the plate wait for themselves to take their swings at the ball while the umpire looks on. / Sitēji pie šķīvja gaida paši, lai veiktu savas šūpoles pie bumbas, kamēr tiesnesis skatās.\n"},
...
def gpt_annotating_multiprocess_lv(args: argparse.Namespace) -> None:
    # Define data_dict
    ...
    for result in results:
        ...
        data_dict_lv['image_names'] += result[1]['image_names'] # result[1] is data_dict_lv
        data_dict_lv['captions'] += result[1]['captions']
        data_dict_lv['all_captions'] += result[1]['all_captions']
        data_dict_lv['caption_numbers'] += result[1]['caption_numbers']
    ...
    for idx in tqdm(range(len(data_dict_lv['captions'])), desc='Tokenizing Latvian captions'):
        cap = data_dict_lv['captions'][idx]
        tokenized = lv_tokenizer(cap, padding='max_length', truncation=True,
                                  max_length=args.max_seq_len, return_tensors='pt')
        data_dict_lv['input_ids'].append(tokenized['input_ids'].squeeze())
    ...
    assert len(data_dict_en['image_names']) == len(data_dict_lv['image_names']), f"len(data_dict_en['image_names']):{len(data_dict_en['image_names'])} != len(data_dict_lv['image_names']):{len(data_dict_lv['image_names'])}"
    assert len(data_dict_lv['image_names']) == len(data_dict_lv['captions']) == len(data_dict_lv['all_captions']) == len(data_dict_lv['caption_numbers']) == len(data_dict_lv['input_ids']), f"data_dict_lv lengths are not equal"

    # Save data_dict_en & data_dict_lv as pickle file
    if args.gpt_model_version == 'gpt-3.5-turbo':
        save_name_en = 'train_GPT35_EN.pkl'
        save_name_lv = 'train_GPT35_LV.pkl'
    elif args.gpt_model_version == 'gpt-4':
        save_name_en = 'train_GPT4_EN.pkl'
        save_name_lv = 'train_GPT4_LV.pkl'
    ...
    with open(os.path.join(preprocessed_path, save_name_lv), 'wb') as f:
        pickle.dump(data_dict_lv, f)
        print(f"Saved {save_name_lv} in {preprocessed_path}")
    ...
    for idx in range(len(image_names_sublist)):
        ...
        while True:
            try:
                ...
                result_sentences = []
                result_sentences.append({"en": gold_caption, "lv": gpt_sentences[0]})
                for i in range(1, len(gpt_sentences)):
                    result_sentences.append({"en": gpt_sentences[i].split(" / ")[0], "lv": gpt_sentences[i].split(" / ")[1]})
            except KeyboardInterrupt as k:
        ...
        # Append to data_dict_en & data_dict_lv
        for i in range(len(result_sentences)):
            ...
            # Append to data_dict_lv
            subset_dict_lv['image_names'].append(image_name)
            subset_dict_lv['captions'].append(result_sentences[i]['lv'])
            subset_dict_lv['caption_numbers'].append(i+1)
            subset_dict_lv['all_captions'].append([result_sentences[i]['lv'] for i in range(len(result_sentences))])

        if tqdm_bar.n + NUM_PROCESS <= tqdm_bar.total:
            tqdm_bar.update(NUM_PROCESS)

    return subset_dict_en, subset_dict_lv # return data_dict_en & data_dict_lv
