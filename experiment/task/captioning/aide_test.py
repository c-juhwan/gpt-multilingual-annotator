...
def testing(args: argparse.Namespace) -> None:
    ...
    dataset_test = CaptioningDataset(args, os.path.join(args.preprocess_path, args.task, args.task_dataset, 'test_ORIGINAL_PL.pkl'), 'test')
    bart_src_lang, bart_tgt_lang = 'pl_PL', 'pl_PL'
    ...
    for test_iter_idx, data_dicts in enumerate(tqdm(dataloader_test, total=len(dataloader_test), desc=f'Testing')):
        ...
        for each_pred_sentence, each_reference in zip(batch_pred_sentences, all_caption):
            ...
            if '</s>' in each_pred_sentence: # facebook/bart-base & vinai/bartpho-syllable
